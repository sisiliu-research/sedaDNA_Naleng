...
dir.create("11_ngsLCA_L30_post/03_offset/01Tables")
dir.create("11_ngsLCA_L30_post/03_offset/01Figures")

#==== load ngs, Source: Dataset 2 ====
indf=read.csv(paste0("11_ngsLCA_L30_post/00_data/Tables/RAW_COUNT_AQUATIC.csv"), check.names = F, row.names = 2)
...
mod1 = stackedsdm(sp_mvabund, ~ Glacier_mass + Land_use + offset(log(sizeFactors)), data=envi, family = "negative.binomial")
...
# biplot 
pdf(paste0("11_ngsLCA_L30_post/03_offset/01Figures/cord_bioplot.pdf"), width = 22, height = 16)  # Open PDF file
par(mfrow=c(1,2))
# samples
...
title(main = "Hypothesis (H1)\nGlacier mass + Land use + offset(log(sizeFactors))", cex.main = 0.5)
dev.off()
...
for (j in abun_seq) {
  ...
  mod3=stackedsdm(sub_sp_mvabund, ~ Glacier_mass + Land_use + offset(log(sizeFactors)), data=envi, family = "negative.binomial")
  ...
  write.csv(Edges, paste0("11_ngsLCA_L30_post/03_offset/01Tables/", occ, "-Edges.csv"))
  # positive links
  ...
  for (resi in res) {
    ...
    if (!is.null(all_mem_cluster)) {
      ...
      write.csv(agg, paste0("11_ngsLCA_L30_post/03_offset/01Tables/", occ, "-member_cluster-resi-", resi,".csv"))
...
# output: marginal network can be generated when j = 0, but the identical resi of H0 and H1 is found when j = 50.

#==== Network with abundance > 50 and resi = 0.52 (the lowest one) ====
i=50
...
mod5=stackedsdm(sub_sp_mvabund, ~ Glacier_mass + Land_use + offset(log(sizeFactors)), data=envi, family = "negative.binomial")
...
g2 = delete.vertices(best_pov_g, isolated)
# 
res=0.52 # the lowest resi of H1 is identical to H0's.
for (resi in res) {
  print(resi)
  all_mem_cluster=NULL
  all_modularity=NULL
...
#==== degree and keystone ====
cluster=read.csv(paste0("11_ngsLCA_L30_post/03_offset/01Tables/", occ, "-member_cluster-resi-", resi, ".csv"), row.names = 1)
...
write.csv(metacluster, paste0("11_ngsLCA_L30_post/03_offset/01Tables/", occ, "-resi-", resi, "-metacluster.csv"))
write.csv(taxacluster, paste0("11_ngsLCA_L30_post/03_offset/01Tables/", occ, "-resi-", resi, "-taxacluster.csv"))

# keystone species
keystone_sp=taxacluster[taxacluster$hs_cluster >= 0.8, ]
write.csv(keystone_sp, paste0("11_ngsLCA_L30_post/03_offset/01Tables/", occ, "-resi-", resi, "-keystone.csv"))
#== END ==  
