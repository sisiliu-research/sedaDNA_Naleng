...
length(unique(indf[indf$rank == "Species", "taxon"])) #20
length(unique(indf[indf$rank == "Genus", "taxon"])) #10
length(unique(indf[indf$rank == "Family", "taxon"])) #9
#
...
rarecurve(df, step = 20, sample = raremax, col = "blue", cex = .8, label = T)
title(paste0("Rare curve based on min rowSum: ", raremax, " for ", ecogroup))
dev.off()

# sequence
basecount=seq(100, 150, 200)
for (i in basecount) {
  # check if higher base count higher species coverage
  pdf(file = paste0("step03_assigned/02_rarefy/outFigures/", ngs, "-", ecogroup, "_rarecurve_", i, ".pdf"))
  rarecurve(df, step = 20, sample = i, col = "blue", cex = 0.8, label = T)
  title(paste0("Rare curve based on: ", i, " for ", ecogroup))
  dev.off()
}
