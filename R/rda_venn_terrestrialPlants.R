...
dir.create("11_ngsLCA_L30_post/04_rda/00Tables")
dir.create("11_ngsLCA_L30_post/04_rda/00Figures")
...
envi=read.csv(paste0("11_ngsLCA_L30_post/04_rda/", eco,"_envi_", trans, ".csv"), row.names = 1)[c("Temperature", "Glaciers_area", "Permafrost_catchment", "Herbivory", "Land_use")]
...
envi$Herbivory <- decostand(envi$Herbivory, method = "standardize")
envi$Land_use <- decostand(envi$Land_use, method = "standardize")

#==== Load NGS data (Source: sheet = PLANT_RAW_COUNT in Dataset 1, please run normalization.R first) ====
if (is.null(neta)) {
...
#==== VIF to check inner correlation ====
envi.rda=rda(t.spe~., envi) # temperature explains higher variations than one of cryospheric factors, include
vif_df=round(vif.cca(envi.rda), 1)
write.csv(vif_df, paste0("11_ngsLCA_L30_post/04_rda/00Tables/", orgs, "_", trans, "_vif_", ngs, "-", eco,  "-r1.csv"))

envi.rda=rda(t.spe~., envi[-3])
vif_df=round(vif.cca(envi.rda), 1)
write.csv(vif_df, paste0("11_ngsLCA_L30_post/04_rda/00Tables/", orgs, "_", trans, "_vif_", ngs, "-", eco,  "-r2.csv"))

#====  rda using envis with vif <= 3 ====
envi.sig=envi[, c("glaciers_area", "Permafrost_catchment", "Herbivory", "Land_use")]
nams <- list(c("glaciers_area", "Permafrost_catchment"), "Herbivory", "Land_use")
# Single variance
meta.pj.var=NULL
for (i in 1:length(nams)) {
  print(nams[[i]])
  ...
  modi=rda(form.i, data=envi.sig)
  # anova table and adjust R^2 %
  an.modi=data.frame(anova(modi), adj.r.squared = round(RsquareAdj(modi)$adj.r.squared*100, 3), call=as.character(modi$call)[2])
  meta.pj.var=rbind(meta.pj.var, an.modi)
}
write.csv(meta.pj.var, paste0("11_ngsLCA_L30_post/04_rda/00Tables/", orgs, "_", trans, "_vif_pjRDA_", ngs, "-", eco, ".csv"))


#==== Joint envi, Total explained % ====
...
# out
write.csv(an.mod, paste0("11_ngsLCA_L30_post/04_rda/00Tables/", orgs, "_", trans, "_vif_jRDA_", ngs, "-", eco, ".csv"))
write.csv(an.adjR2, paste0("11_ngsLCA_L30_post/04_rda/00Tables/", orgs, "_", trans, "_vif_jRDA_adjR2_", ngs, "-", eco, ".csv"))
write.csv(an.axis, paste0("11_ngsLCA_L30_post/04_rda/00Tables/", orgs, "_", trans, "_vif_jRDA_axis_", ngs, "-", eco, ".csv"))
write.csv(an.margin, paste0("11_ngsLCA_L30_post/04_rda/00Tables/", orgs, "_", trans, "_vif_jRDA_margin_", ngs, "-", eco, ".csv"))
...
spe.part <- varpart(t.spe, X1 + X2, X3, X4)
# Partition the variation in community composition
indfract=spe.part$part$indfract
write.csv(indfract, paste0("11_ngsLCA_L30_post/04_rda/00Tables/", orgs, "_", trans, "_indfract_", ngs, "-", eco, ".csv"))
#
pdf(file = paste0("11_ngsLCA_L30_post/04_rda/00Figures/", orgs, "_", trans, "_venn_", ngs, "-", eco,  ".pdf"), width = 15, height = 10)
...
write.csv(meta.cond.var, paste0("11_ngsLCA_L30_post/04_rda/00Tables/", orgs, "_", trans, "_venn_", ngs, "-", eco, ".csv"))
# save image to submit
save.image(file = paste0("11_ngsLCA_L30_post/04_rda/00Tables/", orgs, "_", trans, "_VarPartitioning_", ngs, "_", eco, ".RData"))
#==== END ====
